...
export default function AgentSystem() {
  ...
  function routeMessage(text: string) {
    ...
    const controlling = [
      "umsatz", "kosten", "budget", "marge", "kennzahl", "zahlen", "food waste",
      "gewinn", "verlust", "abweichung", "finanzen", "controlling", "wirtschaftlich",
      "einnahmen", "ausgaben", "rechnung", "preis", "tarif", "analyse", "ebitda",
    ];
    const filialen = [
      "filiale", "standort", "filialen", "personal", "öffnungszeit", "sortiment",
      "café", "produktion", "mitarbeiter", "schicht", "verkauf", "laden", "geschäft",
      "performance", "kundenfeedback",
    ];
    const admin = [
      "reklamation", "dokument", "vorlage", "brief", "kommunikation", "termin",
      "organisation", "ablauf", "digitalisierung", "prozess", "email", "e-mail",
      "hallo", "hilfe", "schreiben", "formulierung",
    ];
  ...
  return (
    <div style={s.root}>
        ...
        {messages.length === 0 && (
          <div style={s.empty}>
            <div style={s.emptyIcon}>
              <img src="/leon.png" alt="Leon" style={{ width: 80, height: 80, borderRadius: "50%", objectFit: "cover" }} />
            </div>
            <div style={s.emptyTitle}>Guten Tag, ich bin Leon.</div>
            <div style={s.emptySub}>
              Ich leite Ihre Anfrage automatisch an Lorena, Sabrina oder Mirjam weiter.
            </div>
            ...
            <div key={i} style={s.assistantRow}>
              {ag && (
                <div style={s.agentHeader}>
                  <img src={ag.image} alt={ag.name} style={{ width: 38, height: 38, borderRadius: "50%", objectFit: "cover" }} />
                  <div>
              ...
              <div style={{ ...s.assistantBubble, borderLeftColor: ag?.accent || "#555" }}>
      ...
      <div style={s.inputArea}>
        <textarea style={s.textarea} placeholder="Ihre Frage an das Team..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} rows={1} />
        <button style={{ ...s.sendBtn, opacity: loading || !input.trim() ? 0.4 : 1, cursor: loading || !input.trim() ? "not-allowed" : "pointer" }} onClick={handleSend} disabled={loading || !input.trim()}>
          ↑
        </button>
      </div>
    </div>
...
function TeamCard({ agent, isActive }: { agent: (typeof AGENTS)[keyof typeof AGENTS]; isActive: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "9px", padding: "8px 12px", borderRadius: "12px", border: "1px solid", borderColor: isActive ? agent.accent : "#1e1e1e", background: isActive ? `${agent.accent}12` : "#111", transform: isActive ? "translateY(-2px)" : "none", boxShadow: isActive ? `0 4px 18px ${agent.accent}20` : "none", transition: "all 0.25s ease", minWidth: "130px" }}>
      <div style={{ position: "relative", flexShrink: 0 }}>
        <img src={agent.image} alt={agent.name} style={{ width: 48, height: 48, borderRadius: "50%", objectFit: "cover", border: isActive ? `2px solid ${agent.accent}` : "2px solid #1e1e1e", transition: "border-color 0.25s ease" }} />
        {isActive && <div style={{ position: "absolute", bottom: 1, right: 1, width: 10, height: 10, borderRadius: "50%", background: agent.accent, border: "2px solid #0d0d0d" }} />}
      </div>
      <div>
        <div style={{ fontWeight: "600", fontSize: "13px", color: isActive ? agent.accent : "#bbb", transition: "color 0.2s" }}>{agent.name}</div>
        <div style={{ fontSize: "10px", color: "#444", fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.03em" }}>{agent.role}</div>
        <div style={{ fontSize: "10px", color: "#2e2e2e", fontStyle: "italic" }}>{agent.animal}</div>
      </div>
    </div>
  );
}
...
const s = {
  root: { display: "flex", flexDirection: "column" as const, height: "100vh", background: "#0d0d0d", color: "#e8e8e8", fontFamily: "'IBM Plex Sans', sans-serif", fontSize: "14px" },
  header: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid #1a1a1a", background: "#0f0f0f", flexWrap: "wrap" as const, gap: "12px" },
  headerLeft: { display: "flex", alignItems: "center", gap: "12px" },
  logo: { width: "36px", height: "36px", background: "#e8c84a", color: "#0d0d0d", borderRadius: "6px", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "700", fontFamily: "'IBM Plex Mono', monospace", fontSize: "13px" },
  headerTitle: { fontWeight: "600", fontSize: "15px" },
  headerSub: { fontSize: "11px", color: "#555", fontFamily: "'IBM Plex Mono', monospace" },
  teamRow: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" as const },
  divider: { width: "1px", height: "52px", background: "#1e1e1e", margin: "0 4px" },
  memoryBar: { display: "flex", alignItems: "center", gap: "8px", padding: "5px 20px", background: "#0a0a0a", borderBottom: "1px solid #141414", fontSize: "11px", fontFamily: "'IBM Plex Mono', monospace" },
  memoryDot: { width: "6px", height: "6px", background: "#e8c84a", borderRadius: "50%", display: "inline-block" },
  memoryText: { color: "#3a3a3a" },
  clearBtn: { background: "transparent", border: "1px solid #1e1e1e", borderRadius: "4px", color: "#3a3a3a", padding: "2px 8px", fontSize: "10px", cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace" },
  messages: { flex: 1, overflowY: "auto" as const, padding: "24px", display: "flex", flexDirection: "column" as const, gap: "24px" },
  empty: { margin: "auto", textAlign: "center" as const, maxWidth: "440px", padding: "32px 0", display: "flex", flexDirection: "column" as const, alignItems: "center", gap: "12px" },
  emptyIcon: { fontSize: "48px" },
  emptyTitle: { fontSize: "20px", fontWeight: "600", color: "#ddd" },
  emptySub: { color: "#555", lineHeight: "1.6", fontSize: "13px" },
  exampleGrid: { display: "flex", flexDirection: "column" as const, gap: "8px", width: "100%", marginTop: "8px" },
  example: { padding: "10px 14px", background: "#141414", border: "1px solid #1e1e1e", borderRadius: "8px", cursor: "pointer", color: "#666", fontSize: "12px", textAlign: "left" as const },
  userRow: { display: "flex", justifyContent: "flex-end" },
  userBubble: { maxWidth: "70%", background: "#1a1a1a", border: "1px solid #222", borderRadius: "14px 14px 2px 14px", padding: "10px 14px", lineHeight: "1.6", color: "#ddd" },
  assistantRow: { display: "flex", flexDirection: "column" as const, gap: "8px", maxWidth: "82%" },
  agentHeader: { display: "flex", alignItems: "flex-start", gap: "10px" },
  agentName: { fontWeight: "600", fontSize: "13px" },
  agentRole: { color: "#444", fontSize: "12px", fontFamily: "'IBM Plex Mono', monospace" },
  agentGrund: { color: "#333", fontSize: "11px", marginTop: "2px", fontStyle: "italic" as const },
  assistantBubble: { background: "#111", border: "1px solid #1a1a1a", borderLeft: "3px solid", borderRadius: "2px 14px 14px 14px", padding: "12px 16px", lineHeight: "1.75", color: "#ccc" },
  loadingBubble: { background: "#111", border: "1px solid #1a1a1a", borderRadius: "14px", padding: "14px 18px", display: "flex", gap: "5px", alignItems: "center", width: "fit-content" },
  dot: { width: "6px", height: "6px", background: "#333", borderRadius: "50%", display: "inline-block", animation: "blink 1.2s infinite ease-in-out" },
  inputArea: { padding: "14px 20px", borderTop: "1px solid #1a1a1a", background: "#0f0f0f", display: "flex", gap: "10px", alignItems: "flex-end" },
  textarea: { flex: 1, background: "#141414", border: "1px solid #1e1e1e", borderRadius: "12px", padding: "10px 14px", color: "#ddd", fontSize: "14px", lineHeight: "1.5", maxHeight: "120px" },
  sendBtn: { width: "40px", height: "40px", background: "#e8c84a", color: "#0d0d0d", border: "none", borderRadius: "12px", fontSize: "18px", fontWeight: "700", display: "flex", alignItems: "center", justifyContent: "center", transition: "opacity 0.2s", flexShrink: 0 },
};
